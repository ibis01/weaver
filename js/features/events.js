...
W.events = (() => {
  ...
  function computeConfidence(signal) {
    ...
    // Corroboration count: currently not available; default to 1
    const corroboration = 1;
    const completeness = 0.8; // assume moderate completeness
    const interpretation = 0.7; // assume moderate model confidence
    // Formula: reliability × freshness × (1 + (corroboration-1)*0.1) × completeness × interpretation
    let confidence = reliability * freshness * (1 + (corroboration - 1) * 0.1) * completeness * interpretation;
  ...
  function normalize(raw, type) {
    ...
    const symbol = String(raw.symbol || raw.id || raw.coin_id || "").toUpperCase();
    const title = String(raw.title || raw.headline || raw.name || "Market Event");
    if (!title) return null;

    // Build minimal AssetId
    const assetId = {
    ...
    // Compute confidence and store in signal (for later use)
  ...
  function collectThesisHealthEvents() {
    ...
    try {
      ...
      // Get current market data for thesis assets
      const assetIds = activeTheses.map(t => t.assetId || t.symbol).filter(Boolean);
      let priceMap = {};
      if (assetIds.length) {
        // Try to get current prices via API
        W.api.markets(assetIds.join(',')).then(markets => {
          markets.forEach(m => { priceMap[m.id] = m.current_price; });
        }).catch(() => {});
      }

      activeTheses.forEach(thesis => {
        // Get price for the thesis asset
        const price = priceMap[thesis.coingeckoId] || priceMap[thesis.symbol?.toLowerCase()] || null;
  ...
  async function collectEvents() {
    ...
    // 3. Deduplicate by asset + type, keep highest confidence
    const seen = new Map();
    ...
    // 4. Sort by confidence descending (optional)
...
console.log("[Events] Live event collector loaded (with confidence model).");