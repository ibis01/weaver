...
W.news = (() => {
      ...
      FEEDS.map(async ([name, url]) => {
        ...
        xml.querySelectorAll("item").forEach((it) => {
          ...
          all.push({
            ...
            source: name,
            published_on:
              Date.parse(it.querySelector("pubDate")?.textContent || "") / 1000,
  ...
  function summarize(n) {
    const text = (n.body || "")
      ...
      .trim();
    if (!text)
      return `No inline body — headline reads <b>${sentiment(n.title)}</b>. Use the source link for the full story.`;
  ...
  function openReader(n) {
    ...
    const m = W.ui.modal({
      title: `<span class="tag rank">${n.source}</span> <span class="muted small">${ago(n.published_on)}</span>`,
      body: `<div class="reader">
        ${n.image ? `<img class="reader-hero" src="${n.image}" onerror="this.remove()">` : ""}
        <h2 class="reader-title">${n.title}</h2>
        <div class="reader-meta"><span class="tag ${s}">${s}</span><span class="muted small">${n.source}</span></div>
        <div class="ai-brief">🤖 <b>Weaver brief:</b> ${summarize(n)}</div>
        <div class="mt">${
          paras
            .slice(0, 8)
            .map((p) => `<p>${p}</p>`)
        ...
        <div class="qa mt">
          <a class="btn primary" target="_blank" href="${n.url}">Read full article ↗</a>
          <button class="btn" id="r-save">${isSaved(n.id) ? "🔖 Saved — remove" : "🔖 Save for later"}</button>
        </div>
      </div>`,
    });
  ...
  async function render(view) {
    view.innerHTML = `
      <div class="card"><h3>📰 Newsroom</h3>
        ...
        <div id="n-brief" class="mt"></div>
      </div>
      <div id="n-list">${W.ui.spinner()}</div>`;

    const [api, rss] = await Promise.allSettled([apiNews(), rssNews()]);
    ITEMS = [
    ...
    view.querySelector("#n-brief").innerHTML =
      `<div class="ai-brief">🤖 <b>Weaver News Brief:</b> scanned ${ITEMS.length} articles from ${[...new Set(ITEMS.map((i) => i.source))].length} sources — tone is <b>${mood}</b> (${counts.bullish} bullish · ${counts.bearish} bearish · ${counts.neutral} neutral). Top themes: ${topThemes(ITEMS)}.</div>`;

    let filter = "All";
    const drawFilters = () => {
      const sources = ["All", "Saved", ...new Set(ITEMS.map((i) => i.source))];
      view.querySelector("#n-filters").innerHTML = sources
        .map(
          (s) =>
            `<button class="chip ${s === filter ? "active" : ""}" data-s="${s}">${s === "Saved" ? `🔖 Saved (${savedList().length})` : s}</button>`,
        )
        .join("");
      ...
      view.querySelector("#n-list").innerHTML = list
        .map((n, k) => {
          const s = sentiment(n.title),
            ...
            saved = isSaved(n.id);
          return `<div class="card news-card ${read ? "read" : ""}" data-k="${k}">
          ${n.image ? `<img src="${n.image}" onerror="this.remove()">` : ""}
          <div style="flex:1"><div class="news-title">${n.title}</div>
          <div class="muted small">${n.source} · ${ago(n.published_on)}</div>
          <span class="tag ${s}">${s}</span>${read ? '<span class="tag buy">✓ read</span>' : ""}${saved ? '<span class="tag rank">🔖 saved</span>' : ""}</div>
          <div style="display:flex;flex-direction:column;gap:6px">
            <button class="icon-btn ${saved ? "save-on" : ""}" data-save="${k}" title="Save for later">🔖</button>
            <button class="icon-btn" data-ext="${k}" title="Open original">↗</button>
          </div>
        </div>`;
        })
        .join("");
