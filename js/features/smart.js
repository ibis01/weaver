...
W.smart = (() => {
  ...
  const qtyOf = t => {
    const raw = typeof t.total === 'object' ? (t.total?.value||'0') : (t.total||'0');
    const dec = parseInt(t.token?.decimals || (typeof t.total==='object' ? t.total?.decimals : 18) || 18, 10);
    return parseFloat(raw)/Math.pow(10, dec);
  };

  function analyze(transfers, addr, pm, nowPrice){
    const week = Date.now()-7*864e5;
    let bal=0, cost=0, realized=0, invested=0, in7=0;
    transfers.slice().reverse().forEach(t => {           // oldest → newest
      const qty = qtyOf(t), ts = new Date(t.timestamp).getTime();
      const p = pm[new Date(ts).toDateString()] ?? nowPrice;
      if ((t.to?.hash||'').toLowerCase() === addr.toLowerCase()){
        bal+=qty; cost+=qty*p; invested+=qty*p; if (ts>=week) in7+=qty;
      } else {
        const q=Math.min(qty,bal), avg = bal>0 ? cost/bal : p;
        realized += q*(p-avg); cost-=q*avg; bal-=qty; if (ts>=week) in7-=qty;
      }
    });
  ...
  async function scan(view){
    ...
    try {
      ...
      if (!contract){ body.innerHTML = W.ui.empty('🧠','No Ethereum contract for this token','Smart scanning supports ERC-20 tokens.'); return; }
      const cur = W.currency();
      const nowPrice = coin.market_data?.current_price?.[cur] || 0;
      const chart = await W.api.chart(scanCoin.id, 365);
      const pm = {}; chart.prices.forEach(([ts,p]) => pm[new Date(ts).toDateString()] = p);
      ...
      const results = (await Promise.allSettled(top.map(h =>
        fetch(`${BSCOUT}/addresses/${h.address.hash}/token-transfers?token=${contract}`).then(j)
          .then(d => ({ addr:h.address.hash, raw:h.value, s:analyze(d.items||[], h.address.hash, pm, nowPrice) }))
      ...
      body.querySelectorAll('[data-track]').forEach(b => b.onclick = () => {
        W.whales.track(b.dataset.track, 'Smart: '+scanCoin.symbol.toUpperCase()+' '+short(b.dataset.track))
          ? W.ui.toast('Added to Whale Tracker 🐋','ok') : W.ui.toast('Already tracked','warn');
      });
    } catch(e){
  ...
  async function render(view){
    ...
    view.querySelector('#sm-go').onclick = () => { if (!scanCoin) return W.ui.toast('Pick a token first','warn'); scan(view); };
  }
