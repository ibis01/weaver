...
describe("User Calibration Metric", () => {
  ...
  it("scores overconfidence lower than well-calibrated decisions", () => {
    ...
    const goodScore = global.W.calibration.forAsset({ symbol: "BTC" }).score;

    const bad = [];
    for (let i = 0; i < 12; i++) bad.push(mk(0.9, i % 2, i));
    global.W.store.set("test_journal", bad);
    const badScore = global.W.calibration.forAsset({ symbol: "BTC" }).score;
  ...
  it("does not expose a confidence-multiplying API", () => {
    // Design rule encoded as a test: calibration never modifies
    // evidence.confidence. See calibration.js header and §2.9.
